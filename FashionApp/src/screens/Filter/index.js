                ...
                <View style={styles.content_container}>
                        ...
                        {colorData.map((item, index) => {
                            return (
                                <TouchableOpacity key={index} style={[styles.color, { backgroundColor: item.color }]}
                                    ...
                                    {colorSelected === index ? Icons.Icons({ name: "check", height: 9.17, width: 11.91 }) : null}
                                </TouchableOpacity>
                            )
                        })}
                    </View>
                    ...
                    <View style={styles.options_container}>
                        {sizeData.map((item, index) => {
                            return (
                                <TouchableOpacity key={index} style={[styles.color,
                                {
                                    backgroundColor: sizeSelected === index ? Colors.black : Colors.white,
                                    borderWidth: 1
                                }]}
                                    onPress={() => setSizeSelected(item.id)}>
                                    <Text style={[styles.text_size, {
                                        color: sizeSelected === index ? Colors.white : Colors.black,
                                    }]}>{item.size}</Text>
                                </TouchableOpacity>
                            )
                        })}
